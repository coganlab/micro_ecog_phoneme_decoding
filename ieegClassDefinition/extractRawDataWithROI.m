...
arguments
    ...
    options.remNoiseThreshold double = 10; % noise threshold to remove noisy trials
...
for iSubject = 1:numSubjects
    ...
    chanIdx = currentSubject.goodChannels;
    ...
    ieegEpoch = trialIEEG(Trials(trials2select), chanIdx, options.Epoch, options.Time .* 1000);
    ieegEpoch = permute(ieegEpoch, [2, 1, 3]);
    
    % Removing noisy trials
    if options.remNoiseThreshold > 0
        [~, goodtrialIds] = remove_bad_trials(ieegEpoch, options.remNoiseThreshold);
    ...
    for iChan = 1:size(ieegEpoch,1)
        ...
        ieegEpoch(iChan,~goodtrialIds(iChan,:),:) = 0;
    end
